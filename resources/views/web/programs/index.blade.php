        ...
        <div class="container">
            ...
            <div id="program_filter">
                <div class="row text-center d-block date-filter-lr">
                    <a href='javascript:;' data-date="{{ Carbon\Carbon::now()->subMonth(1)->format('Y-m') }}" class='pre-date'><</a>
                        {{ $date ? Carbon\Carbon::create($date)->format('M Y') : Carbon\Carbon::now()->format('M Y') }}
                    <a href='javascript:;' data-date="{{ Carbon\Carbon::now()->addMonth(1)->format('Y-m') }}" class='next-date'>></a>
                </div>
                <div class="row">
                    @if (!$programs->isEmpty())
...
@push('script')
    <script>
        ...
        $(document).on('click', '.pre-date', function() {
            var date = $(this).attr("data-date");
